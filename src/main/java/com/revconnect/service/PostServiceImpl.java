...
public class PostServiceImpl implements PostService {
    ...
    public Page<Post> getFeed(Long userId, int page, int size) {
        ...
        for (Connection connection : sentConnections) {
            feedUsers.add(connection.getReceiver());
        }

        for (Connection connection : receivedConnections) {
            feedUsers.add(connection.getSender());
        }

        // Also include users the current user follows
        List<Follow> followedList = followRepository.findByFollower(user);
        for (Follow follow : followedList) {
            User followed = follow.getFollowing();
            if (!feedUsers.contains(followed)) {
                feedUsers.add(followed);
            }
        }
